...
# @Last Modified time: 2019-02-26 18:40:00
# -*- coding: utf-8 -*-
# @Author: lidong
...
def matrix_generation():
    ...
    padding1=torch.zeros(hr_feature.shape[0],1,hr_feature.shape[2],scale).float().cuda()
    padding2=torch.zeros(hr_feature.shape[0],1,scale,hr_feature.shape[3]).float().cuda()
    return distance_matrix.repeat(hr_feature.shape[0],1,hr_feature.shape[-2]//scale,hr_feature.shape[-1]//scale).float().cuda()
...
class context_mapping(nn.Module):
    ...
    def forward(self, lr_feature, hr_feature,lr_feature_r, hr_feature_r):
        ...
        # #return mapping,mapping_r,mapping_l,mapping_t,mapping_b
        # return torch.chunk(mapping_norm*mapping_all,5,dim=1),torch.chunk(mapping_norm_target*mapping_all_target,3,dim=1)
        return weight1,weights1_target
class cmfsm(nn.Module):
    ...
    def forward(self, left, right):
        ...
        mapping,mapping_target=self.context_mapping(refimg_fea,all_feature,targetimg_fea,all_feature_right)
        # matching
        cost = Variable(
        ...
        mapping_target_volume=mapping.view(cost1.shape[0],1,cost1.shape[2],cost1.shape[3]) \
                            .expand(cost1.shape[0],cost1.shape[1],cost1.shape[2],cost1.shape[3])
        ...
        for i in range(self.maxdisp):
            if i>0:
                ...
                mapping_target_volume[:,0,:,:]*=mapping_target


        fused_cost_target=cost1*mapping_target_volume
        ...
        #target
        fused_cost_target=cost2*mapping_target_volume
        ...
        fused_cost_target=cost3*mapping_target_volume
