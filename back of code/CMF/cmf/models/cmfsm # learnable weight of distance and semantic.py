...
# @Last Modified time: 2018-10-16 10:47:57
# -*- coding: utf-8 -*-
# @Author: lidong
...
class context_mapping(nn.Module):
    def __init__(self):
        ...
        self.s1=nn.Conv2d(2, 1, kernel_size=1, stride=1, padding=0,
                               bias=False,dilation=1)
        # self.s2=nn.Conv2d(1, 1, kernel_size=1, stride=1, padding=0,
        #                        bias=False,dilation=1)
        #self.s1.weight.data.fill_(1)
    ...
    def forward(self, lr_feature, hr_feature):
        ...
        if scale%2==0:
            x=torch.arange(-scale//2,scale//2+1)
            ...
            distance_matrix=torch.cat([distance_matrix,torch.pow(distance_matrix[0]-distance_matrix[1],2).unsqueeze(0)],0)
        else:
        ...
        s1=self.s1(torch.ones(1,2,1,1).cuda())
        #s2=self.s2(torch.ones(1,1,1,1).cuda())
        print(self.s1.weight.data.cpu())
        #mapping=(self.s1(weights1)+self.s2(weights2))
        mapping=self.s1(torch.cat([weights1,weights2],1))
        return mapping
class four_related_context_mapping(nn.Module):
    ...
    def forward(self, lr_feature, hr_feature):
        ...
        if scale%2==0:
            ...
            distance_matrix=x.expand(scale,scale).unsqueeze(0)
            distance_matrix=torch.cat([distance_matrix,distance_matrix.transpose(2,1)],0)
            distance_matrix=torch.cat([distance_matrix,torch.pow(distance_matrix[0]-distance_matrix[1],2)],0)
        else:
            x=torch.arange(-scale//2,scale//2+1).unsqueeze(0)
            distance_matrix=x.expand(scale,scale).unsqueeze(0)
            distance_matrix=torch.cat([distance_matrix,distance_matrix.transpose(2,1)],0)
        distance_matrix=distance_matrix.repeat(1,hr_feature.shape[-2]//scale,hr_feature.shape[-1]//scale)
        lr_feature=lr_feature.unsqueeze(-1).expand(lr_feature.shape[0],lr_feature.shape[1],lr_feature.shape[2],lr_feature.shape[3],scale) \
                                     .contiguous().view(lr_feature.shape[0],lr_feature.shape[1],lr_feature.shape[2],lr_feature.shape[3]*scale) \
                                  .unsqueeze(-2).expand(lr_feature.shape[0],lr_feature.shape[1],lr_feature.shape[2],scale,lr_feature.shape[3]*scale) \
                                  .contiguous().view(lr_feature.shape[0],lr_feature.shape[1],lr_feature.shape[2]*scale,lr_feature.shape[3]*scale)
        #128
        representation=torch.cat([lr_feature,hr_feature,lr_feature*hr_feature,torch.norm(lr_feature-hr_feature,dim=1).unsqueeze(1)])
        weights1=self.similarity1(representation)
        weights2=self.similarity2(distance_matrix)
        mapping=(self.s1*self.similarity1*self.s2*self.similarity2)/(self.s1+self.s2)
        bottmom_right_feature=torch.cat([lr_feature[scale/2:],lr_feature[-scale/2:]],1)
        b_r_representation=torch.cat([bottmom_right_feature,hr_feature,bottmom_right_feature*hr_feature,torch.norm(bottmom_right_feature-hr_feature,dim=1).unsqueeze(1)])
        weights1=self.similarity1(b_r_representation)
        weights2=self.similarity2(distance_matrix)
        mapping=(self.s1*self.similarity1*self.s2*self.similarity2)/(self.s1+self.s2)        
        return mapping
