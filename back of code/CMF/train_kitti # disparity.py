...
# @Last Modified time: 2018-11-15 14:37:52
import sys
import torch
import visdom
import argparse
...
def train(args):
    ...
    if args.visdom:
        vis = visdom.Visdom(env='kitti_sub_4_disparity')
    ...
    for epoch in range(trained, args.n_epoch):
        ...
        if error<best_error:
            ...
            torch.save(state, "disparity_{}_{}_{}_{}_best_model.pkl".format(epoch,args.arch,args.dataset,best_error))
        if epoch%15==0:
            state = {'epoch': epoch+1,
             'model_state': model.state_dict(),
             'optimizer_state': optimizer.state_dict(),
...
if __name__ == '__main__':
    ...
    parser.add_argument('--resume', nargs='?', type=str, default='/home/lidong/Documents/CMF/disparity_480_cmfsm_kitti_2.661407928466797_best_model.pkl',
