...
# @Last Modified time: 2019-02-16 11:36:43
...
# output_dir=r'/home/lidong/Documents/datasets/kitti12/'
# left_dir=r'/home/dataset/KITTI2012/training/colored_0/'
# right_dir=r'/home/dataset/KITTI2012/training/colored_1/'
# disparity_dir=r'/home/dataset/KITTI2012/training/disp_occ/'
...
#     l_image=np.array(cv2.imread(os.path.join(left_dir,left_image[f])))[:,:,::-1].astype('float32')
#     r_image=np.array(cv2.imread(os.path.join(right_dir,right_image[f])))[:,:,::-1].astype('float32')
#     disparity=np.array(cv2.imread(os.path.join(disparity_dir,disparity_image[f]),-1)).astype('float32')/256
...
# a=450
output_dir=r'/home/lidong/Documents/datasets/kitti12/'
left_dir=r'/home/dataset/KITTI2012/testing/colored_0/'
right_dir=r'/home/dataset/KITTI2012/testing/colored_1/'
disparity_dir=r'/home/dataset/KITTI2012/training/disp_occ/'
left_image=os.listdir(left_dir)
#left_image.sort()
right_image=os.listdir(right_dir)
...
for f in range(length):
    l_image=np.array(cv2.imread(os.path.join(left_dir,left_image[f])))[:,:,::-1].astype('float32')
    r_image=np.array(cv2.imread(os.path.join(right_dir,right_image[f])))[:,:,::-1].astype('float32')
    disparity=np.array(cv2.imread(os.path.join(left_dir,left_image[f])))[...,0].astype('float32')
    print(os.path.join(left_dir,left_image[f]),os.path.join(right_dir,right_image[f]))
